...
화이트칼라 작업 영역(이메일, 보고서, PPT, 계약서, 이력서, 투자 메모, 사내 공지 등)을 한 단어라도 언급하면, 그 영역 전용 Claude 프로젝트 system instruction을 5-Color Harness 방법론으로 즉시 생성해 주는 Claude Code Skill입니다.

Claude Code 세션에서 `/5color` 형태로 호출하거나, 작업 영역을 한 마디 언급하면 자동 트리거됩니다. 생성된 마크다운을 Claude나 ChatGPT의 프로젝트 지침 박스에 그대로 붙여 넣으시면, 그 프로젝트에서 만들어지는 모든 산출물이 자동으로 5색 평가 레이어를 통과합니다.
...
- [라우팅·합격선·종결 체·em dash](#라우팅합격선종결-체em-dash)
- [폴더 구조](#폴더-구조)
- [평가와 트러블슈팅](#평가와-트러블슈팅)
- [라이선스](#라이선스)
...
| 색 | 역할 | 인원 | 정체 |
|----|------|------|------|
| BLACK | 수행자 | 1명 | 그 작업 영역 베테랑 15년차+ |
| RED | 이성 비평가 | 1명 | 논리·근거·전제·결론 타당성 |
| SILVER | 분야 전문가 비평가 | 1명 | 베테랑이 첫눈에 짚는 그 분야 디테일 |
| BLUE | 공감 비평가 | 1명 | 수신자 감정·톤·진정성 |
| GOLD | 보고 받는 이/독자/대상 | 1명 | 시간·장소·행위·심리로 정의된 단일 독자 |

세 가지 핵심 원칙입니다.

- 세 비평가는 서로 다른 사고 모드입니다. 이성·분야 전문성·공감 셋이 같은 산출물을 다르게 봅니다. 한 색이 다른 색의 자리에 침범하면(예. RED가 감정으로, BLUE가 논리 분석으로 흐르면) 그 발언은 무효입니다.
- GOLD는 인구통계가 아니라 상황으로 정의합니다. "30대 직장인"이 아니라 "월요일 9시 30분, 이사회 직전에 5장짜리 보고서를 처음 펼친 CEO"처럼 시간, 장소, 행위, 심리가 들어갑니다.
- BLACK은 항상 15년차 이상으로 캐스팅하고 톤, 제약, 분량, 종결 체를 한 줄로 못 박습니다.
...
## 라우팅·합격선·종결 체·em dash

스킬의 카테고리 라우팅(8개), 합격선 차등(9.0/9.5/9.7), 종결 체 가이드, em dash 0개 정책은 [SKILL.md](SKILL.md)에 본문 표로 들어 있습니다. 여기 README에서 다시 옮겨 적으면 두 곳에서 어긋날 위험이 있어 한 줄 포인터로만 둡니다.

요약만 보시려면.

- **카테고리 8개.** 외부/내부 커뮤니케이션, 의사결정 문서, 발표 자료, 분석·법무, 마케팅, AI 프롬프트, 개인용. 표에 없는 과제는 외부 커뮤니케이션이 최종 fallback. 자세한 표는 [SKILL.md의 1차 라우팅](SKILL.md)을 보십니다.
- **합격선 3군.** 9.0(시·소설·블로그처럼 작가 톤이 자산), 9.5(표준), 9.7(계약서·이사회·IR·위기 대응처럼 결함이 곧 손실). 9.7군은 그 영역 흔한 실패 문구를 직접 인용 금지로 박습니다.
- **종결 체.** 작업 영역마다 다릅니다(보고서 `~이다`, 이메일·CS·사과문 `~합니다·~드립니다`, PPT 헤드라인 `~함·~임`, 뉴스레터 `~입니다`, 보도자료·법률 의견서 `~다·~이다`, SNS·블로그·시·소설은 작성자 톤). 한 산출물 안에 두 체 섞이면 위반.
- **em dash 0개.** 한국어 호흡이 끊기는 자리. 본문에 `—`, `–` 한 개라도 있으면 자동 fail이며 마침표·쉼표·괄호·콜론으로 즉시 교체합니다.
...
`references/<카테고리>.md`에는 매핑 표(작업 영역별 RED 이성·SILVER 분야 전문가·BLUE 공감 시각, 합격선, 기준 장면), 문체와 금지 뱅크(분량과 종결 체 포함), fallback 합성 알고리즘이 들어 있습니다. 작업 영역을 추가하시려면 이 파일들만 손보시면 됩니다.
...
- 스킬이 트리거되지 않습니다. 폴더 경로와 이름(`~/.claude/skills/5-color-harness/`)을 확인하시고 Claude Code 세션을 새로 여십니다.
...
- 페르소나 정체성 침범. RED가 분야 디테일이나 감정으로, BLUE가 논리 분석으로 흐르면 무효 처리합니다. 다음 메시지에 "RED는 이성, BLUE는 공감만"을 명시하시면 재캐스팅됩니다.
- em dash가 산출물에 보입니다. "본문 em dash 0개로 다시"를 한 줄 던지시면 마침표·쉼표·괄호로 즉시 교체됩니다.
